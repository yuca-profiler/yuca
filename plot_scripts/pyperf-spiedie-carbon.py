...
def process(report, signal, name):
    df = report.copy()
...
def get_all_reports(amortized=True):
    dfs = []
...
reports = [
    pd.read_csv("data-ghpc005-pyperformance-instance-0/agg_symbols.csv"),
    pd.read_csv("data-ghpc005-pyperformance-instance-1/agg_symbols.csv"),
    pd.read_csv("data-ghpc005-pyperformance-instance-2/agg_symbols.csv"),
]
...
all_reports = get_all_reports(False)
...
plt.savefig(
    f'plots/{plots_dir}/{machine}_pyperf_stacked_operational.pdf',
    bbox_inches='tight'
)
...
plt.savefig(
    f'plots/{plots_dir}/{machine}_pyperf_stacked_amortized.pdf',
    bbox_inches='tight'
)
...
plt.savefig(
    f'plots/{plots_dir}/{machine}_pyperf_carbon_percentage.pdf',
    bbox_inches='tight'
)
